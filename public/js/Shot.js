class Shot {
  ...
  init(x, y, d, r, type, team, rank, host) {
    ...
    this.e = Date.now()+(type === 'torpedo' ? 500 : 0);
  ...
  update() {
    ...
    const time = 1//Math.floor((Date.now()-this.e)/15),
    const x = this.target ? this.target.x-this.offset[0] : time*this.xm+this.sx, y = this.target ? this.target.y-this.offset[1] : time*this.ym+this.sy, x1 = Math.floor(x/100), x2 = Math.floor((x+10)/100), y1 = Math.floor(y/100), y2 = Math.floor((y+10)/100);
