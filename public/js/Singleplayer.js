class Singleplayer extends Engine {
  constructor(level) {
    ...
    if (level > this.levels.length || level < 1) level = 1;
    super([this.levels[level-1]], Math.floor((level-1)/20)*5);
    if (this.levels[level-1][0] === 3) {
      this.survivalTimeout = setTimeout(() => this.victory(), 60000);
      this.startTime = Date.now();
    }
    if (this.levels[level-1][0] === 1) {
      let ez = 0;
  ...
  ondeath(t, m) {
    ...
    if (t.username !== PixelTanks.user.username) {
      ...
      if (e === 0 && !this.victoryTimeout) {
        if (this.levels[level-1][0] === 1) this.global = 'All enemies defeated!';
        this.victory();
      }
      if (this.levels[level-1][0] === 1) this.global = e+' Enemies remaining!';
